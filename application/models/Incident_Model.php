...
Class Incident_Model extends CI_Model{
    ...
    // Update Incident
    public function update_incident($update_data,$id){
      $this->db->where("id",$id);
      return $this->db->update("erms_risk_incident",$update_data);  
    }

    // Get all the Incident
    public function get_incident($inc_id,$rep_id,$esc_id){  
        $this->db->select('eri.*,eu.email,ed.email as mail,ers.section_name,erd.directorate_name');
        ...
        $this->db->join('erms_user as ed', 'eri.esc_user_id = ed.id');
        $this->db->where('eri.active',1);
